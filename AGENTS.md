...
- For requested local installation or updates, read `.agents/skills/local-install/SKILL.md`. Finishing development does not authorize installation or app restart.
...
- Prepare a local bundle without launching: `CODEXBAR_SIGNING=adhoc CODEXBAR_SKIP_LAUNCH_SMOKE=1 ./Scripts/package_app.sh release`. Follow `local-install` for resource validation and installation.
...
- Run focused tests for changed behavior locally. Full `make test` and release packaging run in `.github/workflows/personal-macos.yml` on GitHub macOS runners; record the source SHA and remote result before installation. Documentation/workflow preparation needs static validation only. Do not default to a local full suite.
- After code changes, run relevant local static checks; require the remote `make check` job to pass before installation. Fix issues in changed code; report unrelated baseline failures.
...
- Validate requested UI/runtime behavior against the freshly built bundle. For installed-version verification, launch `/Applications/CodexBar.app` and verify the running executable path and bundle metadata as described in `local-install`.
