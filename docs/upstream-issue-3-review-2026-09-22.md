...
日期：2026-09-22。当前状态：三个范围已实施、通过云端验证并安装，用户确认原问题已解决。以下审核正文保留实施前的判断；最新结果见文末「实施与安装结果」。
...
2026-09-22 完成选择性移植，未执行全量上游同步：

- Kimi 额度解析：ratio pools、严格零占位修正、安全数字转换，保留缺失窗口语义。
- Kimi 展示与认证：耗尽月额度自动展示、Desktop 会话失效恢复、显式来源与 Manual/Off 边界。自动浏览器导入限定 Chrome。
- Codex 刷新：401/403 区分、有效账号快照及原测量时间保留、包装网络错误和取消识别。修正审核发现的 403 正文误判，并补充回归测试。

最终 Standards 与 Spec 复审均无确认问题。云端 [run 35708094643](https://github.com/allgemeiner-intellekt/CodexBar/actions/runs/35708094643)，attempt 1，源码 `8d661b1431cf9ef3dd0de14342719efbec143427` 的 `make check`、两个完整测试分片及个人打包全部成功。构建期间修复了既有文档检查器对两个个人维护 skill 链接的精确白名单缺失。

产物版本为 `0.56.4 (135)`，arm64、ad-hoc 签名。下载后验证了压缩包及主程序 SHA-256、签名、源码提交、构建时间、应用身份和官方自动更新关闭设置：

- 压缩包 SHA-256：`db27a95af68c3b10011e5fc87e3afbc42475c1cb8b4bab7b30c95bb3588db7e8`。
- 主程序 SHA-256：`23a58b433ee54dbdb54563de411b0799728d8e352236f8b4af02c135ae489ce3`。

经用户授权替换 `/Applications/CodexBar.app`，启动后核实运行路径及源码版本；旧安装保留在下载产物目录的 `previous-installation/CodexBar.app`。用户随后确认此前问题已修复、可以正常使用。这是用户当前使用场景的验收，不代表所有账号、地区、浏览器或 widget 场景均已验证。

安装包对应上述构建 SHA；其后的收尾提交仅补充维护文档，不另行构建或替换应用。构建跟进自动化已暂停。

Issue #3 继续作为上游更新汇总保留开放。地区支持、会员名与 CLI 版本、额外 credits、其他供应商及 Quotio 等暂缓项不因此视为已接收。监控仍按 `main..upstream` 统计，选择性移植不会消除原上游提交；按上次评估版本增量提醒的改进留待单独修改。
